# AI Coding Guide Book 📚

> 面向初级 iOS 开发者的 AI 编程实践指南  
> 从环境搭建到多 Agent 协作，系统掌握 AI 辅助开发的核心方法

---

## 🎯 这是什么

`AI Coding Guide Book` 是一份围绕 **AI 辅助编程** 的系统化教程，重点面向：

- 有基础 Swift / Xcode 使用经验的 iOS 开发者
- 想把 AI 真正用进日常开发流程的独立开发者
- 对 Claude Code、Cursor、Codex、MCP、Agent 协作感兴趣的学习者

这不是一份单纯的工具说明书，而是一套从 **入门体验、核心概念、实战方法，到高阶工作流** 的完整学习路径。

---

## 👥 适合谁读

这份指南尤其适合下面几类读者：

- 想开始尝试 AI 编程，但不知道从哪里入手
- 会写 iOS App，但还不会高效和 AI 协作
- 已经在用 Cursor / Claude Code / Codex，但流程不稳定
- 希望建立更清晰的 Prompt、验收、修复和自动化工作流
- 想把 AI 工具从“偶尔试试”变成“每天都能用”

---

## 🚀 你将学到什么

通过这份指南，你将逐步掌握：

- 如何搭建完整的 AI 编程环境
- 如何选择适合自己的 AI 编程工具
- 如何把需求清楚地交给 AI
- 如何评估、修正和验收 AI 生成的代码
- 如何把 AI 工具用到真实 iOS 开发场景中
- 如何进一步构建多 Agent 协作和自动化工作流

---

## 📚 内容结构

本书按学习路径分为四个阶段：**基础入门 → 核心概念 → 工具实战 → 高阶工作流**。

### 01 基础篇
帮助你完成环境搭建、工具安装，并做出第一个 AI 辅助开发项目。

包含内容：
- 环境搭建
- 工具安装
- 第一个项目

入口：
- [基础篇：环境搭建](./docs/基础篇/01-环境搭建/README.md)
- [基础篇：工具安装](./docs/基础篇/02-工具安装/README.md)
- [基础篇：第一个项目](./docs/基础篇/03-第一个项目/README.md)

### 02 进阶篇
理解 AI 编程背后的关键概念，掌握更稳定、更高质量的协作方式。

包含内容：
- Agent 概念
- MCP 协议
- Prompt 技巧
- Rules 与 Skill

入口：
- [Agent 概念](./docs/进阶篇/01-Agent概念/README.md)
- [MCP 协议](./docs/进阶篇/02-MCP协议/README.md)
- [Prompt 技巧](./docs/进阶篇/03-Prompt技巧/README.md)
- [Rules 与 Skill](./docs/进阶篇/04-Rules与Skill/README.md)

### 03 实战篇
聚焦真实开发场景，学习如何把 AI 工具真正用进项目。

包含内容：
- Claude Code 实战
- Cursor 实战
- Codex 实战

入口：
- [Claude Code 实战](./docs/实战篇/01-ClaudeCode实战/README.md)
- [Cursor 实战](./docs/实战篇/02-Cursor实战/README.md)
- [Codex 实战](./docs/实战篇/03-Codex实战/README.md)

### 04 大师篇
面向更复杂的协作与自动化场景，进一步拓展 AI 编程工作流。

包含内容：
- 多 Agent 协作
- OpenClaw 远程 Coding
- 高级工作流

入口：
- [多 Agent 协作](./docs/大师篇/01-多Agent协作/README.md)
- [OpenClaw 远程 Coding](./docs/大师篇/02-OpenClaw远程Coding/README.md)
- [高级工作流](./docs/大师篇/03-高级工作流/README.md)

---

## 🧭 推荐阅读路径

### 新手入门
按顺序阅读以下内容：

1. [环境搭建](./docs/基础篇/01-环境搭建/README.md)
2. [工具安装](./docs/基础篇/02-工具安装/README.md)
3. [第一个项目](./docs/基础篇/03-第一个项目/README.md)
4. [Prompt 技巧](./docs/进阶篇/03-Prompt技巧/README.md)
5. [Claude Code / Cursor / Codex 实战](./docs/实战篇/01-ClaudeCode实战/README.md)

### 已有开发经验
如果你已经有一定 iOS 或 AI 工具使用经验，建议从这里开始：

1. [进阶篇](./docs/进阶篇/01-Agent概念/README.md)
2. [实战篇](./docs/实战篇/01-ClaudeCode实战/README.md)
3. [大师篇](./docs/大师篇/01-多Agent协作/README.md)

---

## 🚀 快速导航

| 模块 | 说明 | 入口 |
|------|------|------|
| 基础篇 | 环境、工具、入门项目 | [开始学习](./docs/基础篇/01-环境搭建/README.md) |
| 进阶篇 | 核心概念、Prompt、Rules | [继续深入](./docs/进阶篇/01-Agent概念/README.md) |
| 实战篇 | 工具实战与开发场景 | [查看实战](./docs/实战篇/01-ClaudeCode实战/README.md) |
| 大师篇 | 多 Agent、远程协作、工作流 | [进阶提升](./docs/大师篇/01-多Agent协作/README.md) |
| 示例模板 | AGENTS、Swift 模板 | [查看 examples](./examples/) |
| 资源汇总 | 官方文档与外部资料 | [查看 resources](./resources/README.md) |

---

## 🛠 仓库结构

```text
aicoding-guide-book/
├── README.md
├── SUMMARY.md
├── AGENTS.md
├── CHANGELOG.md
├── CONTRIBUTING.md
├── LICENSE
├── docs/
│   ├── 基础篇/
│   ├── 进阶篇/
│   ├── 实战篇/
│   └── 大师篇/
├── examples/
└── resources/
```

---

## ✨ 这份指南的特点

### 1. 面向真实开发，而不是空谈概念
本项目强调“能落地”。每一部分都尽量围绕真实开发问题展开，而不是只介绍工具功能。

### 2. 面向初级 iOS 开发者
你不需要先成为 AI 专家。只要你已经会一些 Swift / Xcode，就可以逐步把 AI 引入自己的开发流程。

### 3. 强调“生成之后的判断力”
AI 能生成很多代码，但真正重要的是：

- 你是否能判断它对不对
- 你是否知道哪里要修改
- 你是否能建立自己的验收标准

### 4. 关注长期可持续的工作流
除了“怎么生成代码”，这份指南也会覆盖：

- Prompt 组织方式
- 代码验收方法
- 安全边界设置
- 多工具协作
- 自动化工作流

---

## 📦 仓库内还包含什么

除了主线教程，这个仓库还包含这些内容：

- [AGENTS 配置模板](./examples/AGENTS-examples.md)
- [Swift 代码模板](./examples/swift-templates.md)
- [故障排查指南](./docs/FAQ.md)
- [快速参考卡片](./docs/cheat-sheet.md)
- [最佳实践案例](./docs/best-practices.md)
- [术语表](./docs/glossary.md)
- [学习路径图](./docs/learning-path.md)
- [资源汇总](./resources/README.md)

---

## 📝 阅读建议

为了获得更好的学习效果，建议你采用下面的方式：

### 不要只读，要跟着做
每学完一章，至少自己动手完成一个小练习。

### 不要只让 AI 生成，要学会验收
重点不是“让 AI 写得更快”，而是“写完后你知道怎么判断”。

### 不要一口吃完
你不需要一次性读完整本。更好的方式是：

- 先上手
- 再解决当前最需要的问题
- 最后回头补概念和体系

---

## 🤝 贡献方式

欢迎一起完善这个项目。

你可以通过这些方式参与：

- 发现错误 → 提 Issue
- 补充案例 → 提 PR
- 推荐资源 → 提 Issue 或 Discussion
- 提出新章节建议 → 提 Feature Request

相关文档：

- [贡献指南](./CONTRIBUTING.md)
- [变更日志](./CHANGELOG.md)

---

## 📜 License

本项目采用 [MIT License](./LICENSE)。

---

## 🙏 致谢

感谢以下项目和资源带来的启发：

- [OpenAI Codex](https://developers.openai.com/codex/cli)
- [Model Context Protocol](https://modelcontextprotocol.io/)
- [OpenClaw](https://docs.openclaw.ai/)
- [AGENTS.md Best Practices](https://agentsmd.io/)

---

## ⭐ 开始阅读

从这里开始最合适：

- [基础篇：环境搭建](./docs/基础篇/01-环境搭建/README.md)
- [基础篇：工具安装](./docs/基础篇/02-工具安装/README.md)
- [基础篇：第一个项目](./docs/基础篇/03-第一个项目/README.md)

如果这个项目对你有帮助，欢迎 Star ⭐
